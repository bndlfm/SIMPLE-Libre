...
class TestAmbushRules(unittest.TestCase):
    ...
    def test_ambush_multi_faction_selection(self):
        # Add BOTH Govt and DR
        self.sp.pieces[0] = 1 # Govt Troop
        self.sp.pieces[5] = 1 # DR Underground

        sa_idx = self.env._ops_action_base + self.OP_AMBUSH_M26 * 13 + 0
        self.env.step(sa_idx)

        # Environment should now be in PHASE_CHOOSE_TARGET_FACTION
        self.assertEqual(self.env.phase, 6, "Should pause for faction selection when multiple enemies present")

        # Verify that both Govt(0) and DR(2) are legal targets
        masks = self.env.action_masks()
        self.assertTrue(masks[self.env._target_faction_action_base + 0])
        self.assertTrue(masks[self.env._target_faction_action_base + 2])
        self.assertFalse(masks[self.env._target_faction_action_base + 1]) # Cannot target self
        self.assertFalse(masks[self.env._target_faction_action_base + 3]) # Syndicate not present

        # Select DR
        self.env.step(self.env._target_faction_action_base + 2)

        # Environment should now be in PHASE_CHOOSE_TARGET_PIECE
        self.assertEqual(self.env.phase, 8, "Should proceed to piece selection after faction selection")

        masks = self.env.action_masks()
        self.assertTrue(masks[self.env._target_piece_action_base + 5], "DR piece should be selectable")
        self.assertFalse(masks[self.env._target_piece_action_base + 0], "Govt piece should NOT be selectable after DR chosen")

        self.env.step(self.env._target_piece_action_base + 5)

        # Should remove DR piece
        self.assertEqual(self.sp.pieces[5], 0, "DR Guerrilla should be removed")
        self.assertEqual(self.sp.pieces[0], 1, "Govt Troop should NOT be removed")
